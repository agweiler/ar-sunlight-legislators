...
class Test
  ...
  def self.list_state_reps(state)
    legislator = Legislator.select('first_name', 'last_name', 'party').where("state = '#{state}'") #.where("title = 'Rep'")
  ...
  def self.list_state_senators(state)
    legislator = Legislator.select('first_name', 'last_name', 'party').where("state = '#{state}'").where("title = 'Sen'")
    puts 'Senators:'
    puts '------------------------'
  ...
  def self.list_senators_by_party(party)
    legislator = Legislator.select('first_name', 'last_name', 'party').where(" party = '#{party}'").where("title = 'Sen'")
    puts 'Senators:'
    puts '------------------------'
  ...
  def self.list_reps_by_party(party)
    legislator = Legislator.select('first_name', 'last_name', 'party').where(" party = '#{party}'").where("title = 'Rep'")
    puts 'House Representatives:'
    puts '------------------------'
